        ...
        <>
            ...
            <LiveKitRoom token={viewerToken} serverUrl={process.env.NEXT_PUBLIC_LIVEKIT_WS_URL} className={cn("grid grid-cols-1 lg:gap-y-0 lg:grid-cols-3 2xl:grid-cols-6 h-full ",collapsed && "lg:grid-cols-2 2xl:grid-cols-2")}>
                ...
                <div className={cn("col-span-1",collapsed && "hidden")}>
                    <Chat viewerName={viewerName} hostName={user.username} hostIdentity={user.id} isFollowing={isFollowing} 
                    isChatEnabled={stream.isChatEnabled} isChatDelayed={stream.isChatDelayed} isChatFollowersOnly={stream.isChatFollowersOnly}/>
                </div>
            </LiveKitRoom>
        </>
    )
